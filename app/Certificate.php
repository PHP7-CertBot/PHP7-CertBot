...
class Certificate extends Model
{
    ...
    {
        return preg_split('/\s+/', $this->domains);
    }

    public function subjectAlternativeNames($domains)
    {
        $san = [];
        foreach ($domains as $domain) {
            // Technically I should add email and IPv4/v6 support here, but LetsEncrypt doesnt support it so meh
            $san[] = ['dNSName'   => $domain];
        }
    ...
    public function generateRequest()
    {
        ...
        // Craft the DN as CN=firstDomainName
...
//		$csr->setExtension('id-ce-extKeyUsage', ['id-kp-serverAuth']);
        ...
        $altnames = $this->subjectAlternativeNames($domains);
