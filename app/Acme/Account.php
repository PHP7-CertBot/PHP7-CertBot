...
class Account extends Model
{
	...
	public function acmeClientLog() { return $this->client->log(); }
	...
	public function postNewReg() {
		if ($this->status != 'unregistered') { throw new \Exception('Account status is not unregistered, it is ' . $this->status); }
		...
		if(!$response['id']) { throw new \Exception('registration update error, no acme ca registration id recieved in response'); }
	...
	public function postUpdateReg() {
		if ($this->status != 'registered') { throw new \Exception("account status is not registered, it is " . $this->status);}
	...
	public function getAcmeChallenge($subject) {
		...
		if(!$challenge) { throw new \Exception('No suitable challenges for selected authtype were identified'); }
	...
	public function getDnsClient() {
		...
		}else if ($this->authprovider == "verisign") {
